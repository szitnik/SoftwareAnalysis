...
/* package */ abstract class AbstractValidatingLambdaMetafactory {
    ...
    AbstractValidatingLambdaMetafactory(MethodHandles.Lookup caller,
        ...
        this.implKind = (implInfo.getReferenceKind() == MethodHandleInfo.REF_invokeSpecial)
                        ...
                        : implInfo.getReferenceKind(); // @@@ Temporary work-around pending resolution of 8005119
    ...
    void validateMetafactoryArgs() throws LambdaConversionException {
        ...
        if (implArity + receiverArity != capturedArity + samArity) {
            throw new LambdaConversionException(
                    String.format("Incorrect number of parameters for %s method %s; %d captured parameters, %d functional interface parameters, %d implementation parameters",
                                  implIsInstanceMethod ? "instance" : "static", implInfo,
                                  capturedArity, samArity, implArity));
        }
        if (instantiatedArity != samArity) {
            throw new LambdaConversionException(
                    String.format("Incorrect number of parameters for %s method %s; %d functional interface parameters, %d SAM method parameters",
                                  implIsInstanceMethod ? "instance" : "static", implInfo,
